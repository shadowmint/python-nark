...
class TestHelper:
  """ Helper class to find and run pyunit tests 
      ...
      Why use this? Because now are you can go:

      Works in python 2.7?
      run: python2.7 tests/runner.py

      Works in python 3.3?
      run: python3.3 tests/runner.py

      Works in pypy?
      run: pypy tests/runner.py

      You get the idea.
      You can get this working with nose too, it's just more effort.
  """
